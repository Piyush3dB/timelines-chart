...
export default Kapsule({
  ...
  stateInit: {
    ...
    transDuration: 200,   // ms for transition duration
    ...
    function addTooltips() {
      ...
      state.segmentTooltip = d3Tip()
        ...
        .html((event, d) => {
          ...
          console.log(d);
          return '<strong>' + d.label + '.' + d.labelVal + ' </strong>' + state.zDataLabel
            + (normVal?' (<strong>' + Math.round((d.val-state.zColorScale.domain()[0])/normVal*100*100)/100 + '%</strong>)':'') + '<br>'
            + '<strong>St: </strong>' + dateFormat(d.timeRange[0]) + '<br>'
            + '<strong>En: </strong>' + dateFormat(d.timeRange[1]) + '<br>'
            + '<strong>Dr: </strong>' + (dateFormat(d.timeRange[1] - d.timeRange[0]));
        });
